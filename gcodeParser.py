...
class GcodeParser:
    ...
    def parseFile(self, path):
        ...
        self.model.setMeshBBox(self.mesh_BBox)
        self.model.postProcess()
        return self.model

    def setMeshBBox(self, BBox):
        self.mesh_BBox = BBox
    ...
    def calcMetrics(self):
        ...
        for layer in self.layers:
            ...
            for seg in layer.segments:
                ...
                cx = (seg.coords["X"]+coords["X"])/2
                cy = (seg.coords["Y"]+coords["Y"])/2
                cz = (seg.coords["Z"]+coords["Z"])/2

                # calc extrudate
                seg.extrudate = (seg.coords["E"]-coords["E"])

                # calc center of gravity
                if not seg.distance == 0:
        ...
        if self.stl_bbox:
            self.slicer_translation = {
                ...
                "Z": self.bbox.cz() - self.stl_bbox.cz()
            }
        else:
            self.slicer_translation = {'X':0, 'Y':0, 'Z':0}


        # Get cog coordinates in 3D mesh coordinate system
